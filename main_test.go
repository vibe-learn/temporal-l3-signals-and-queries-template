...
func TestTaskQueueDefault(t *testing.T) {
	if os.Getenv("TEMPORAL_TASK_QUEUE") == "" && TaskQueue() != "lesson-l3_signals_and_queries-tq" {
		t.Errorf("TaskQueue() default = %q, want lesson-l3_signals_and_queries-tq", TaskQueue())
	}
}
...
func TestOrderWorkflow(t *testing.T) {
	...
	env.OnActivity(CheckStock, mock.Anything).Return("ok", nil)
	env.OnActivity(Checkout, mock.Anything).Return("ok", nil)
...
func TestIntegration(t *testing.T) {
	...
	// поведения урока «Сигналы и запросы».
}
